...
if __name__ == '__main__':
    ...
    restart_cycles = max(1, int(getattr(args, "lr_restart_cycles", 1) or 1))
    if restart_cycles <= 1:
        print('lr warmup-restart cycles: restart off (single cosine cycle)')
    else:
        print('lr warmup-restart cycles: ', restart_cycles)
    if args.DRL == 'SAC':

...
AUTO_LOGGING_ACCESSORS = {
    "h2_batt": "float(self.h2_batt)",
    "h2_equal": "float(self.h2_equal)",
}

FORBIDDEN_ANALYSIS_ONLY_READS: dict = {}
...
def ensure_logging_contract(
    method_source: str,
    required_info_keys: Optional[Iterable[str]] = None,
) -> str:
    """Add safe fallback logging lines for a small set of known analysis-only keys."""
    if not required_info_keys:
        return method_source

    missing_auto_keys = [
        key
        for key in required_info_keys
        if key in AUTO_LOGGING_ACCESSORS
        and f"'{key}'" not in method_source
        and f'"{key}"' not in method_source
    ]
    if not missing_auto_keys:
        return method_source

    lines = method_source.rstrip("\n").splitlines()
    insert_at = None
    indent = "    "
    ...
    injected = [
        f'{indent}self.info.setdefault("{key}", {AUTO_LOGGING_ACCESSORS[key]})'
        for key in missing_auto_keys
    ]
    patched = lines[:insert_at] + injected + lines[insert_at:]
    return "\n".join(patched).rstrip("\n") + "\n"
...
def validate_method_source(
    ...
    for node in ast.walk(fn):
        ...
        if not isinstance(node.value, ast.Name) or node.value.id != "self":
            continue
        if node.attr in FORBIDDEN_ANALYSIS_ONLY_READS:
            raise PatchError(FORBIDDEN_ANALYSIS_ONLY_READS[node.attr])

    lowered_source = method_source.lower()
    for key, message in FORBIDDEN_ANALYSIS_ONLY_READS.items():
        if f'getattr(self, "{key}"' in lowered_source or f"getattr(self, '{key}'" in lowered_source:
